...
const scoresDB = require("../models/scores");
...
router.get("/reportData/", async (req,res)=>{
    ...
    try{
        ...
        const reply=await(scoresDB.find({$and:[{'stud_id':studId},{'quiz_id':quizId}]}));
        console.log(reply);
        if(reply.length==0){
            const resp =await(scoresDB.create({
                'stud_id':req.query['ID'],
...
router.get("/classReportData/:quizName", async (req,res)=>{
    try{
        const quizName=req.params.quizName;
        var data=await(quizes.find({'name':quizName},{_id:1,'class_id':1}));
        const quizId=(data[0]._id).toString();
        const classId=(data[0].class_id).toString();
        const scores=await(scoresDB.find({'quiz_id':quizId}));
        
        const totalStudents=(await (student.find({'class_id':classId}))).length;
        console.log(scores,totalStudents);
        res.status(200).send({'scores':scores,'totalStudents':totalStudents});
    }catch(err){
        console.log(err);
    }finally{
        console.log("Success");
